...
def test_query_basic(magic):
    ...
    with patch("probing.core.engine.query", return_value=mock_df), patch(
        "probing.repl.query_magic.display"
    ) as mock_display:
...
def test_tables(magic):
    """Test %tables command."""
    with patch("probing.core.engine.query") as mock_query, patch(
        "probing.repl.query_magic.display"
    ):
        magic.tables("")
